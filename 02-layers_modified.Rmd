...
```{R}
password <- read_csv(here::here("data", "passwords.csv"))

str(password)

ggplot(password, aes(category)) +
  geom_bar() +
  coord_flip() 
  
pw_count <- count(password, category)

ggplot(pw_count, aes(fct_reorder(category, n), n)) +
geom_col() +
geom_text(aes(label = n), hjust = -.5, size = 5) +
...
# Helsinki version
sf_hki <- sf::st_sfc(sf::st_point(c(24.9349, 60.1905)), crs = sf::st_crs(sf_world))
...
ggplot(sf_world) +
  ...
  geom_sf(data = sf_hki, 
          ...
          size = 5, 
          stroke = 2) +
coord_sf(crs = "+proj=laea +x_0=0 +y_0=0 +lon_0=0 +lat_0=-20")
```
...
```{R}
sf_finland <- rnaturalearth::ne_countries(country = "Finland" , returnclass = "sf")

# Hires
# sf_finland_hd <- rnaturalearth::ne_countries(country = "Finland", scale = 10, returnclass = "sf")
...
```{R}
# Map of Finland with Helsinki added
ggplot(sf_finland) +
geom_sf(fill = "#CBE7BE") +
geom_sf(data = sf_hki, size = 5) +
coord_sf(crs = "+proj=moll")
